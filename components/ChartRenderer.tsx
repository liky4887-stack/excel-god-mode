...
export const ChartRenderer = forwardRef<Svg, Props>(function ChartRenderer(
  { config, width = 320, height = 240 },
  ref
) {
  const firstSeries = config?.series?.[0];
  const rawData: any[] = firstSeries?.data ?? [];
  const values: number[] = rawData.map((d) => Number(d?.value) || 0);
  const labels: string[] = rawData.map((d) => String(d?.label ?? ''));
  const title: string = config?.title || '';
  const cfgType: string = config?.type || 'bar';

  const type: 'bar' | 'line' | 'pie' =
    cfgType === 'pie' || cfgType === 'donut' ? 'pie'
    : cfgType === 'line' || cfgType === 'area' ? 'line'
    : 'bar';
  ...
  if (type === 'pie') {
    ...
    const r = Math.max(10, Math.min(plotW, plotH) / 2 - 4);
    let angle = -Math.PI / 2;
    return (
      <View style={{ width, height, backgroundColor: '#FFFFFF' }}>
        <Svg ref={ref} width={width} height={height}>
          {title ? (
            <SvgText x={width / 2} y={18} textAnchor="middle" fontSize={13} fontWeight="700" fill="#0F172A">{title}</SvgText>
          ) : null}
          {values.map((v, i) => {
            ...
            angle += sweep;
            return <Path key={`slice-${i}`} d={d} fill={COLORS[i % COLORS.length]} stroke="#FFFFFF" strokeWidth={1.5} />;
          })}
        </Svg>
      </View>
    );
  ...
  return (
    <View style={{ width, height, backgroundColor: '#FFFFFF' }}>
      <Svg ref={ref} width={width} height={height}>
        ...
        {[0, 0.5, 1].map((p, i) => (
          <Line key={`g-${i}`} x1={PAD_L} y1={PAD_T + plotH * (1 - p)} x2={width - PAD_R} y2={PAD_T + plotH * (1 - p)} stroke="#E2E8F0" strokeWidth={1} />
        ))}

        {[0, 0.5, 1].map((p, i) => (
          <SvgText key={`y-${i}`} x={PAD_L - 6} y={PAD_T + plotH * (1 - p) + 4} textAnchor="end" fontSize={9} fill="#64748B">
        ...
        {values.map((_, i) => (
          <SvgText key={`x-${i}`} x={PAD_L + i * slot + slot / 2} y={height - 8} textAnchor="middle" fontSize={8} fill="#64748B">
            {(labels[i] || '').slice(0, 6)}
          </SvgText>
        ))}
      </Svg>
